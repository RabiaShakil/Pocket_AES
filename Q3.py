...
def decryptionAlgorithm(ciphertext, key):
...
def hexDecimal_to_ASCI(hex_string):
    try:
...
if __name__ == '__main__':
    ...
    key = input("Enter the 16-bit key in hexadecimal: ")   
    ...
    for i in range(0, len(text), 4):
        msg = text[i:i + 4]
        decrypted_block = decryptionAlgorithm(msg, key)
        decrypted_text += decrypted_block
        
    decrypted_text = decrypted_text.rstrip('0')

    with open("plain.txt", "w") as output_file:
        decrypted_msg = hexDecimal_to_ASCI(decrypted_text)
        output_file.write(decrypted_msg)

    print("Decrypted result:",decrypted_msg)
